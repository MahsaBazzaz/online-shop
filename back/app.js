...
app.get('/admin/getAllProducts', async(req, res) => {
    ...
    if (login && password && login === auth.login && password === auth.password) {
        //Admin access granted
        const products = await adminService.getAllProducts(req.query.page, req.query.products_in_page);
        res.send(products);
...
app.use('/admin/CreateOrUpdateProduct', async(req, res) => {
    const auth = { login: 'admin', password: 'password' };
    const b64auth = (req.headers.authorization || '').split(' ')[1] || '';
    const [login, password] = Buffer.from(b64auth, 'base64').toString().split(':');
    ...
    if (login && password && login === auth.login && password === auth.password) {
        //Admin access granted
        const result = await adminService.createOrUpdateProduct(req.query.product_id, req.body);
        res.send(result);
